...
function generateSeries(
  ...
  for (let i = 0; i < days; i++) {
    ...
    const v = missing
      ? null
      : +(base + Math.sin(i / 4) * amp + (rand(seed, i) - 0.5) * jitter).toFixed(2);
    arr.push({
...
function ReportsPage() {
  ...
  const ponds = useMemo(
    () => (farm === "all" ? PONDS : PONDS.filter((p) => p.farm_id === farm)),
    [farm],
  );
  const seed = useMemo(
    () => (pond === "all" ? 17 : pond.charCodeAt(1) * 13) + param.charCodeAt(0),
    [pond, param],
  );
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
              ...
              <SelectContent>
                {(Object.keys(PARAM_META) as ParamKey[]).map((k) => (
                  <SelectItem key={k} value={k}>
                    {t(PARAM_META[k].labelEn, PARAM_META[k].labelBn)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            <Select value={reportType} onValueChange={(v) => setReportType(v as ReportKey)}>
              ...
              <SelectContent>
                <SelectItem value="daily">
                  {t("Daily pond summary", "দৈনিক পুকুর সারসংক্ষেপ")}
                </SelectItem>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        ...
        />
        <ScoreTile
          label={t("Parameters in safe range", "নিরাপদ পরিসরে পরামিতি")}
          value={summary.safePct}
          suffix="%"
          icon={<ShieldCheck className="h-4 w-4" />}
          tone={summary.safePct >= 80 ? "good" : "warn"}
        />
        <MetricTile
        ...
        <ScoreTile
          ...
          tone={summary.uptime >= 95 ? "good" : "warn"}
        />
        <ScoreTile
          ...
          tone={summary.calibrated >= 5 ? "good" : "warn"}
        />
      </div>
      ...
      <section>
        <h3 className="mb-3 font-display text-base font-semibold">
          {t("Generate report", "রিপোর্ট তৈরি করুন")}
        </h3>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <ReportCard
...
function TrendChart({
  ...
  title: string;
  data: Array<{ day: string; v: number | null }>;
  param: ParamKey;
        ...
        <ResponsiveContainer>
          <AreaChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            ...
            <Tooltip
              ...
              formatter={(v) => [`${v} ${meta.unit}`, t(meta.labelEn, meta.labelBn)]}
            />
            <ReferenceArea
              y1={meta.safe[0]}
              y2={meta.safe[1]}
