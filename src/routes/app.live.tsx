...
const statusTone: Record<string, { pill: string; dot: string; cardBorder: string; cardBg: string }> = {
  good:            { pill: "bg-emerald-500/10 text-emerald-700 border-emerald-500/30", dot: "bg-emerald-500", cardBorder: "border-border/60",      cardBg: "bg-card" },
  watch:           { pill: "bg-sky-500/10 text-sky-700 border-sky-500/30",             dot: "bg-sky-500",     cardBorder: "border-sky-500/30",     cardBg: "bg-card" },
  warning:         { pill: "bg-amber-500/10 text-amber-700 border-amber-500/30",       dot: "bg-amber-500",   cardBorder: "border-amber-500/40",   cardBg: "bg-amber-500/[0.03]" },
  critical:        { pill: "bg-rose-500/10 text-rose-700 border-rose-500/30",          dot: "bg-rose-500",    cardBorder: "border-rose-500/50",    cardBg: "bg-rose-500/[0.05]" },
  offline:         { pill: "bg-muted text-muted-foreground border-border",             dot: "bg-muted-foreground/50", cardBorder: "border-border", cardBg: "bg-muted/30" },
  calibration_due: { pill: "bg-violet-500/10 text-violet-700 border-violet-500/30",    dot: "bg-violet-500",  cardBorder: "border-violet-500/30",  cardBg: "bg-card" },
};

const riskRank: Record<string, number> = { critical: 0, warning: 1, offline: 2, calibration_due: 3, watch: 4, good: 5 };

function staleness(ts: string, isBn: boolean) {
  const diff = Date.now() - new Date(ts).getTime();
  const mins = Math.floor(diff / 60_000);
  if (mins < 1) return { label: isBn ? "এইমাত্র" : "just now", stale: false, mins };
  if (mins < 60) return { label: isBn ? `${mins} মিনিট আগে` : `${mins}m ago`, stale: mins > 15, mins };
...
function tickPonds(prev: MockPond[]): MockPond[] {
  return prev.map((p) => {
    ...
    const salinity_ppt = p.salinity_ppt == null ? null : +jitter(p.salinity_ppt, 0.05, 0, 40).toFixed(2);
    const ammonia_mg_l = p.ammonia_mg_l == null ? null : +jitter(p.ammonia_mg_l, 0.01, 0, 2).toFixed(3);
    ...
    const newStatus: MockPond["status"] =
      do_mg_l < 3 ? "critical" :
      do_mg_l < 4 || ph > 8.5 || ph < 6.5 ? "warning" :
      p.status === "calibration_due" ? "calibration_due" : "good";
    return {
      ...p,
      do_mg_l, ph, temp_c, turbidity_ntu, salinity_ppt, ammonia_mg_l,
      battery_pct, signal_pct, trend,
      status: newStatus,
      ...
      setPonds((prev) => {
        ...
        prev.forEach((p, i) => {
          if (next[i].status === "critical" && p.status !== "critical") {
            toast.error(isBn ? `${next[i].name}: জরুরি অবস্থা` : `${next[i].name}: critical reading`, {
              description: isBn ? `DO ${next[i].do_mg_l} mg/L — অবিলম্বে ব্যবস্থা নিন` : `DO ${next[i].do_mg_l} mg/L — take action`,
            });
          }
        });
        return next;
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {isBn ? "সকল পুকুরের রিয়েল-টাইম জলগুণ পরিমাপ।" : "Real-time water quality across all ponds."}
          </p>
        </div>
      ...
      <div className="rounded-2xl border border-border/60 bg-card p-3 shadow-soft">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <ControlGroup label={<><ArrowUpDown className="mr-1 inline h-3 w-3" />{isBn ? "সর্ট" : "Sort"}</>}>
...
function FreshnessIndicator({ lastTick, pulse, auto, isBn }: { lastTick: Date; pulse: boolean; auto: boolean; isBn: boolean }) {
  ...
  return (
    <div className={cn(
      ...
      stale ? "border-amber-500/30 bg-amber-500/10 text-amber-700" : "border-emerald-500/30 bg-emerald-500/10 text-emerald-700"
    )}>
      <span className="relative flex h-2 w-2">
        <span className={cn("absolute inline-flex h-full w-full rounded-full opacity-70", auto && !stale ? "bg-emerald-500 animate-ping" : "bg-current opacity-0")} />
        <span className={cn("relative inline-flex h-2 w-2 rounded-full", stale ? "bg-amber-500" : "bg-emerald-500", pulse && "scale-125 transition-transform")} />
      </span>
...
function RiskSummary({ ponds, isBn }: { ponds: MockPond[]; isBn: boolean }) {
  ...
  const items = [
    { key: "critical", label: isBn ? "জরুরি" : "Critical", count: counts.critical, tone: "bg-rose-500/10 text-rose-700 border-rose-500/30", icon: <AlertCircle className="h-3.5 w-3.5" /> },
    { key: "warning", label: isBn ? "সতর্কতা" : "Warning", count: counts.warning, tone: "bg-amber-500/10 text-amber-700 border-amber-500/30", icon: <Waves className="h-3.5 w-3.5" /> },
    { key: "offline", label: isBn ? "অফলাইন" : "Offline", count: counts.offline, tone: "bg-muted text-muted-foreground border-border", icon: <Radio className="h-3.5 w-3.5" /> },
    { key: "good", label: isBn ? "ভালো" : "Good", count: counts.good, tone: "bg-emerald-500/10 text-emerald-700 border-emerald-500/30", icon: <Droplets className="h-3.5 w-3.5" /> },
  ];
  return (
    <div className="flex flex-wrap items-center gap-2">
      {items.map((i) => (
        <span key={i.key} className={cn("inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-semibold", i.tone)}>
          {i.icon}{i.label}<span className="ml-1 rounded-full bg-background/60 px-1.5 tabular-nums">{i.count}</span>
        </span>
      ))}
    </div>
  );
...
function LiveCard({ pond, paramFilter, isBn, pulse }: { pond: MockPond; paramFilter: ParamKey; isBn: boolean; pulse: boolean }) {
  ...
  return (
    <div
      className={cn(
        ...
        tone.cardBorder, tone.cardBg,
        isCritical && "ring-2 ring-rose-500/30",
        isOffline && "opacity-80",
      )}
    >
      {isCritical && (
        <span className="absolute -right-10 -top-10 h-28 w-28 rounded-full bg-rose-500/20 blur-2xl" aria-hidden />
      )}
      {pulse && !isOffline && (
        <span className="absolute right-3 top-3 h-1.5 w-1.5 animate-ping rounded-full bg-sky-500" aria-hidden />
      )}
      ...
      <div className="relative flex items-start justify-between gap-2">
        ...
        <span className={cn("inline-flex shrink-0 items-center gap-1.5 rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider", tone.pill)}>
          <span className={cn("h-1.5 w-1.5 rounded-full", tone.dot)} />
          {isBn
            ? { good: "ভালো", watch: "নজরে", warning: "সতর্কতা", critical: "জরুরি", offline: "অফলাইন", calibration_due: "ক্যালি." }[pond.status]
            : pond.status === "calibration_due" ? "Calib." : pond.status}
        </span>
      </div>
      ...
      <div className="relative mt-3 grid grid-cols-2 gap-2">
        {showParam("do") && (
          <BigReading
            icon={<Droplets className="h-3.5 w-3.5" />} label="DO" unit="mg/L"
            value={pond.do_mg_l} digits={2}
            crit={pond.do_mg_l < 3} warn={pond.do_mg_l < 4} stale={isOffline}
          />
        )}
        {showParam("ph") && (
          <BigReading
            icon={<FlaskConical className="h-3.5 w-3.5" />} label="pH" unit=""
            value={pond.ph} digits={2}
            crit={pond.ph < 6 || pond.ph > 9} warn={pond.ph < 6.5 || pond.ph > 8.2} stale={isOffline}
          />
        )}
        {showParam("temp") && (
          <BigReading
            icon={<Thermometer className="h-3.5 w-3.5" />} label={isBn ? "তাপমাত্রা" : "Temp"} unit="°C"
            value={pond.temp_c} digits={1}
            crit={pond.temp_c > 32} warn={pond.temp_c > 30} stale={isOffline}
          />
        )}
        {showParam("turbidity") && (
          <BigReading
            icon={<CloudFog className="h-3.5 w-3.5" />} label={isBn ? "ঘোলাটে" : "Turb."} unit="NTU"
            value={pond.turbidity_ntu} digits={1}
            warn={pond.turbidity_ntu > 30} crit={pond.turbidity_ntu > 50} stale={isOffline}
          />
        )}
      </div>
      ...
      <div className="relative mt-3 flex items-center justify-between gap-2 rounded-lg border border-border/40 bg-background/40 px-2 py-1.5 text-[11px]">
        <span className="flex items-center gap-1">
          <Battery className={cn("h-3.5 w-3.5", pond.battery_pct < 20 ? "text-amber-600" : pond.battery_pct === 0 ? "text-muted-foreground" : "text-emerald-600")} />
          <span className="font-medium tabular-nums">{pond.battery_pct}%</span>
        </span>
        <span className="flex items-center gap-1">
          <Signal className={cn("h-3.5 w-3.5", pond.signal_pct < 30 ? "text-amber-600" : pond.signal_pct === 0 ? "text-muted-foreground" : "text-sky-600")} />
          <span className="font-medium tabular-nums">{pond.signal_pct}%</span>
        </span>
        <span className={cn("flex items-center gap-1", s.stale ? "text-rose-600 font-semibold" : "text-muted-foreground")}>
          <Clock className="h-3 w-3" />
          {s.label}
          {s.stale && <span className="ml-0.5 rounded bg-rose-500/10 px-1 py-px text-[9px] font-bold uppercase">stale</span>}
        </span>
      </div>

      {/* CTA */}
      <Button asChild size="sm" variant={isCritical ? "default" : "outline"} className={cn("relative mt-3 w-full", isCritical && "bg-rose-600 hover:bg-rose-700")}>
...
function BigReading({
  icon, label, unit, value, digits, warn, crit, stale,
}: {
  icon: React.ReactNode; label: string; unit: string; value: number; digits: number;
  warn?: boolean; crit?: boolean; stale?: boolean;
}) {
  const color = stale ? "text-muted-foreground" : crit ? "text-rose-600" : warn ? "text-amber-600" : "text-foreground";
  const bg = stale ? "bg-muted/40 border-border" : crit ? "bg-rose-500/8 border-rose-500/30" : warn ? "bg-amber-500/8 border-amber-500/30" : "bg-background/60 border-border/40";
  return (
...
function SecondaryReading({ icon, label, value, warn }: { icon: React.ReactNode; label: string; value: string; warn?: boolean }) {
  return (
    <div className={cn("flex items-center justify-between rounded-lg border px-2 py-1.5", warn ? "border-amber-500/30 bg-amber-500/5 text-amber-700" : "border-border/40 bg-background/40")}>
      <span className="flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{icon}{label}</span>
