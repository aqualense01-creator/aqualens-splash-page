...
import {
  AlertTriangle, AlertCircle, CheckCircle2, Cpu, FlaskConical, Search,
  Bell, MessageCircle, StickyNote, Mail, Smartphone, Send,
} from "lucide-react";
...
type Alert = {
  ...
  device: string;
  delivery: { sms: "sent" | "pending" | "failed"; push: "sent" | "pending" | "failed"; email: "sent" | "pending" | "failed" };
};
...
const SEED_ALERTS: Alert[] = [
  {
    id: "a1", category: "critical", status: "open",
    pondId: "p2", pondName: "Pond 2 — Shrimp",
    parameter: "Dissolved O₂", paramKey: "do", currentValue: "2.8", unit: "mg/L",
    safeRange: "5 – 9 mg/L", thresholdLabel: "< 4 mg/L",
    ...
    detectedAt: m(8), device: "AL-SENSE-02",
    delivery: { sms: "sent", push: "sent", email: "sent" },
  },
  {
    id: "a2", category: "warning", status: "open",
    pondId: "p3", pondName: "Pond 3 — Tilapia",
    parameter: "pH", paramKey: "ph", currentValue: "8.6", unit: "",
    safeRange: "6.8 – 8.2", thresholdLabel: "> 8.5",
    ...
    detectedAt: m(35), device: "AL-SENSE-03",
    delivery: { sms: "sent", push: "sent", email: "pending" },
  },
  {
    id: "a3", category: "warning", status: "acknowledged",
    pondId: "p1", pondName: "Pond 1 — Rui",
    parameter: "Temperature", paramKey: "temp", currentValue: "31.8", unit: "°C",
    safeRange: "24 – 31 °C", thresholdLabel: "> 31.5 °C",
    ...
    detectedAt: m(95), device: "AL-SENSE-01",
    delivery: { sms: "sent", push: "sent", email: "sent" },
  },
  {
    id: "a4", category: "device", status: "open",
    pondId: "p5", pondName: "Pond 5 — Carp Mix",
    parameter: "Device offline", paramKey: "device", currentValue: "—", unit: "",
    safeRange: "Online", thresholdLabel: "> 3 hours offline",
    ...
    detectedAt: m(185), device: "AL-SENSE-05",
    delivery: { sms: "sent", push: "failed", email: "sent" },
  },
  {
    id: "a5", category: "calibration", status: "open",
    pondId: "p6", pondName: "Pond 6 — Koi",
    parameter: "pH sensor calibration", paramKey: "calibration", currentValue: "Due", unit: "",
    safeRange: "Calibrated within 30 days", thresholdLabel: "31 days overdue",
    ...
    detectedAt: m(60 * 26), device: "AL-SENSE-06",
    delivery: { sms: "pending", push: "sent", email: "sent" },
  },
  {
    id: "a6", category: "critical", status: "resolved",
    pondId: "p4", pondName: "Pond 4 — Pangas",
    parameter: "Ammonia", paramKey: "amm", currentValue: "0.18", unit: "mg/L",
    safeRange: "0 – 0.2 mg/L", thresholdLabel: "> 0.4 mg/L",
    ...
    detectedAt: m(60 * 12), device: "AL-SENSE-04",
    delivery: { sms: "sent", push: "sent", email: "sent" },
  },
  {
    id: "a7", category: "warning", status: "resolved",
    pondId: "p1", pondName: "Pond 1 — Rui",
    parameter: "Turbidity", paramKey: "ph", currentValue: "62", unit: "NTU",
    safeRange: "10 – 35 NTU", thresholdLabel: "> 60 NTU",
    ...
    detectedAt: m(60 * 48), device: "AL-SENSE-01",
    delivery: { sms: "sent", push: "sent", email: "sent" },
  },
];
...
function AlertsPage() {
  ...
  const counts = useMemo(() => ({
    critical: alerts.filter((a) => a.category === "critical" && a.status !== "resolved").length,
    warning: alerts.filter((a) => a.category === "warning" && a.status !== "resolved").length,
    device: alerts.filter((a) => a.category === "device" && a.status !== "resolved").length,
    calibration: alerts.filter((a) => a.category === "calibration" && a.status !== "resolved").length,
  ...
  const visible = useMemo(() => {
    ...
    return alerts
      ...
      .filter((a) => {
        if (!q) return true;
        return [a.pondName, a.parameter, a.messageEn, a.messageBn, a.device]
          .some((x) => x.toLowerCase().includes(q));
      })
      .sort((a, b) => {
        const rank = (c: AlertCategory) => (c === "critical" ? 0 : c === "warning" ? 1 : c === "device" ? 2 : 3);
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        <MetricTile label={t("Critical", "গুরুতর")} value={counts.critical} accent="text-rose-600" />
        ...
        <MetricTile label={t("Calibration", "ক্যালিব্রেশন")} value={counts.calibration} accent="text-violet-600" />
        <MetricTile label={t("Resolved", "সমাধান হয়েছে")} value={counts.resolved} accent="text-emerald-600" />
      </div>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
          <TabsList className="flex-wrap">
            <TabsTrigger value="all">{t("All", "সব")} ({alerts.length})</TabsTrigger>
            <TabsTrigger value="critical">{t("Critical", "গুরুতর")} ({counts.critical})</TabsTrigger>
            <TabsTrigger value="warning">{t("Warning", "সতর্ক")} ({counts.warning})</TabsTrigger>
            <TabsTrigger value="device">{t("Device", "ডিভাইস")} ({counts.device})</TabsTrigger>
            <TabsTrigger value="calibration">{t("Calibration", "ক্যালিব্রেশন")} ({counts.calibration})</TabsTrigger>
            <TabsTrigger value="resolved">{t("Resolved", "সমাধান হয়েছে")} ({counts.resolved})</TabsTrigger>
          </TabsList>
        </Tabs>
        <div className="relative w-full sm:w-72">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t("Search alerts…", "অ্যালার্ট খুঁজুন…")} className="pl-9" />
        </div>
      </div>
              ...
              <TableBody>
                {visible.map((a) => (
                  <TableRow
                    ...
                    className={cn(
                      "cursor-pointer",
                      a.category === "critical" && a.status !== "resolved" && "bg-rose-500/5 hover:bg-rose-500/10"
                    )}
                  >
                    <TableCell className="font-medium">{a.pondName}</TableCell>
                    <TableCell className="text-muted-foreground">{a.parameter}</TableCell>
                    <TableCell className="tabular-nums">{a.currentValue} {a.unit}</TableCell>
                    <TableCell className="text-muted-foreground">{a.thresholdLabel}</TableCell>
                    <TableCell><SeverityBadge category={a.category} t={t} /></TableCell>
                    <TableCell className="text-xs text-muted-foreground">{timeAgo(a.detectedAt, lang)}</TableCell>
                    <TableCell className="max-w-[260px] truncate text-sm">{t(a.recommendedEn, a.recommendedBn)}</TableCell>
            ...
            {visible.map((a) => (
              <button
                ...
                className={cn(
                  ...
                  a.category === "critical" && a.status !== "resolved" ? "border-rose-500/40 bg-rose-500/5" : "border-border/70"
                )}
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-display text-base font-semibold">{a.pondName}</p>
                    <p className="text-xs text-muted-foreground">{a.parameter} · {a.currentValue} {a.unit}</p>
                  </div>
      ...
      <AlertDetailDrawer
        ...
        onAck={(id) => { update(id, { status: "acknowledged" }); toast.success(t("Alert acknowledged", "অ্যালার্ট স্বীকৃত")); }}
        onResolve={(id) => { update(id, { status: "resolved" }); toast.success(t("Alert resolved", "অ্যালার্ট সমাধান হয়েছে")); setOpenId(null); }}
...
// ----- Severity badge -----
function SeverityBadge({ category, t }: { category: AlertCategory; t: (en: string, bn: string) => string }) {
  const map = {
    critical: { cls: "bg-rose-500/10 text-rose-700 border-rose-500/30", Icon: AlertTriangle, label: t("Critical", "গুরুতর") },
    warning:  { cls: "bg-amber-500/10 text-amber-700 border-amber-500/30", Icon: AlertCircle, label: t("Warning", "সতর্ক") },
    device:   { cls: "bg-sky-500/10 text-sky-700 border-sky-500/30", Icon: Cpu, label: t("Device", "ডিভাইস") },
    calibration: { cls: "bg-violet-500/10 text-violet-700 border-violet-500/30", Icon: FlaskConical, label: t("Calibration", "ক্যালিব্রেশন") },
  }[category];
  const Icon = map.Icon;
  return (
    <span className={cn("inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium uppercase tracking-wider", map.cls)}>
...
function AlertDetailDrawer({
  alert, onClose, onAck, onResolve, onNote, onContact, t, lang,
}: {
  alert: Alert | null;
  onClose: () => void;
      ...
      <SheetContent className="w-full overflow-y-auto sm:max-w-lg">
        <SheetHeader>
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <SheetDescription>{alert.pondName} · {alert.device}</SheetDescription>
            </div>
        ...
        <div className="mt-4 space-y-5">
          ...
          <div className={cn(
            ...
            alert.category === "critical" ? "border-rose-500/40 bg-rose-500/5" : "border-border bg-muted/30"
          )}>
            <p className="font-medium">{lang === "bn" ? alert.messageBn : alert.messageEn}</p>
            <p className="mt-1 text-xs text-muted-foreground">{lang === "bn" ? alert.messageEn : alert.messageBn}</p>
          </div>

          {/* Key facts */}
          <dl className="grid grid-cols-2 gap-3 text-sm">
            <Fact label={t("Current value", "বর্তমান মান")} value={`${alert.currentValue} ${alert.unit}`} />
            <Fact label={t("Safe range", "নিরাপদ পরিসর")} value={alert.safeRange} />
            <Fact label={t("Time detected", "সনাক্তের সময়")} value={new Date(alert.detectedAt).toLocaleString()} />
          ...
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-3">
            ...
            <p className="mt-1 text-sm font-medium">{t(alert.recommendedEn, alert.recommendedBn)}</p>
          </div>

          {/* History chart */}
          <div>
            <p className="mb-2 text-sm font-semibold">{t("Alert history (24h)", "অ্যালার্ট ইতিহাস (২৪ ঘ)")}</p>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <DeliveryPill icon={<Smartphone className="h-3.5 w-3.5" />} label={t("SMS", "এসএমএস")} state={alert.delivery.sms} t={t} />
              <DeliveryPill icon={<Send className="h-3.5 w-3.5" />} label={t("Push", "পুশ")} state={alert.delivery.push} t={t} />
              <DeliveryPill icon={<Mail className="h-3.5 w-3.5" />} label={t("Email", "ইমেইল")} state={alert.delivery.email} t={t} />
            </div>
          </div>
          ...
          <div>
            ...
            <Button size="sm" variant="outline" className="mt-2"
              onClick={() => {
                if (!note.trim()) { toast.error(t("Note cannot be empty", "নোট খালি হতে পারে না")); return; }
                onNote(alert.id, note); setNote("");
              }}>
          ...
          <div className="flex justify-between">
            <Link to="/app/ponds/$pondId" params={{ pondId: alert.pondId }} className="text-xs text-primary hover:underline">
              {t("Open pond detail →", "পুকুর বিস্তারিত দেখুন →")}
            </Link>
            <button onClick={() => onContact(alert.id)} className="inline-flex items-center gap-1 text-xs text-primary hover:underline">
        ...
        <SheetFooter className="mt-6 gap-2">
          ...
          {alert.status !== "resolved" && (
            <Button onClick={() => onResolve(alert.id)}>
              <CheckCircle2 className="mr-1.5 h-3.5 w-3.5" /> {t("Mark resolved", "সমাধান হিসেবে চিহ্নিত")}
            </Button>
          )}
        </SheetFooter>
      </SheetContent>
...
function DeliveryPill({
  icon, label, state, t,
}: { icon: React.ReactNode; label: string; state: "sent" | "pending" | "failed"; t: (en: string, bn: string) => string }) {
  const cls = state === "sent" ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-700"
    : state === "pending" ? "border-amber-500/40 bg-amber-500/10 text-amber-700"
    : "border-rose-500/40 bg-rose-500/10 text-rose-700";
  const stateLabel = state === "sent" ? t("Sent", "পাঠানো")
    : state === "pending" ? t("Pending", "অপেক্ষমাণ") : t("Failed", "ব্যর্থ");
  return (
