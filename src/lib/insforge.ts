...
const MOCK_PONDS_FULL = [
  {
    ...
    stocking_density: 3.5,
    threshold_preset: "standard_carp",
  ...
  },
  {
    ...
    area_m2: 1800,
    depth_m: 1.6,
    stocking_date: "2026-03-15",
    stocking_density: 8,
    threshold_preset: "standard_tilapia",
...
const MOCK_TICKETS = [
  {
    ...
    issue_type: "device",
    priority: "high",
    description: "Oxygen level reading has sudden drop. Check sensor calibration.",
    status: "open",
    subject: "Pond 2 low oxygen warning",
    created_at: m(120),
    updated_at: m(60),
  },
  {
    ...
    issue_type: "device",
    priority: "normal",
    description: "AL-Sense 05 has gone offline since yesterday.",
    status: "open",
    subject: "Device AL-Sense 05 offline",
    created_at: m(480),
    updated_at: m(480),
  },
  {
    ...
    issue_type: "device",
    priority: "low",
    description: "Low battery warning on Pond 3 device.",
    status: "resolved",
    subject: "AL-Sense 03 battery replacement",
    created_at: m(1440),
    updated_at: m(1000),
  },
];
...
const MOCK_MAINTENANCE_LOGS = [
  {
    id: "ml1",
    device_id: "d5",
    technician_id: "demo-user-3",
    visit_type: "troubleshoot",
    notes: "Cleaned algae growth from sensors and verified power connection.",
    performed_at: m(600),
  },
  {
    id: "ml2",
    device_id: "d6",
    technician_id: "demo-user-3",
    visit_type: "calibration",
    notes: "Calibrated pH sensor with 4.0 and 7.0 buffer solutions.",
    performed_at: m(1200),
  },
];
...
const MOCK_ADMIN_SETTINGS_DOCS = [
  {
    id: "doc1",
    key: "general",
    data: { platformName: "Acqua Lence", generalMaintenance: false },
    updated_at: m(1000),
  },
];
...
const MOCK_ALERT_NOTES = [
  {
    ...
    note: "Turned on Pond 2 aerator. Oxygen levels should recover within the hour.",
    created_at: m(3),
  },
];
...
function generateMockReadings() {
  ...
  > = {
    p1: {
      do: [6.2, 6.4, 6.5, 6.6, 6.7, 6.9, 7.0, 6.9, 6.8, 6.8, 6.9, 6.8],
      ph: 7.4,
      temp: 28.6,
      turb: 18,
      amm: 0.08,
    ...
    p2: {
      do: [5.8, 5.4, 5.0, 4.6, 4.1, 3.8, 3.4, 3.1, 3.0, 2.9, 2.8, 2.8],
      ph: 7.1,
      temp: 31.2,
      turb: 42,
      sal: 14.2,
    ...
    p3: {
      do: [5.6, 5.4, 5.2, 5.0, 4.9, 4.8, 4.6, 4.5, 4.4, 4.3, 4.2, 4.2],
      ph: 8.4,
      temp: 30.1,
      turb: 26,
      amm: 0.18,
    ...
    p4: {
      do: [6.9, 7.0, 7.0, 7.1, 7.0, 7.1, 7.2, 7.1, 7.0, 7.1, 7.1, 7.1],
      ph: 7.6,
      temp: 28.9,
      turb: 20,
      amm: 0.05,
    ...
    p5: {
      do: [5.5, 5.5, 5.4, 5.4, 5.4, 5.4, 5.4, 5.4, 5.4, 5.4, 5.4, 5.4],
      ph: 7.3,
      temp: 27.8,
      turb: 22,
      amm: 0.12,
    ...
    p6: {
      do: [6.4, 6.5, 6.6, 6.5, 6.6, 6.5, 6.4, 6.5, 6.5, 6.5, 6.5, 6.5],
      ph: 7.5,
      temp: 28.4,
      turb: 16,
      amm: 0.06,
...
const getTableMockData = (tableName: string): any[] => {
  switch (tableName) {
    case "farms":
      return MOCK_FARMS_FULL;
    case "ponds":
      return MOCK_PONDS_FULL;
    case "devices":
      return MOCK_DEVICES_FULL;
    case "alerts":
      return MOCK_ALERTS_FULL;
    case "profiles":
    ...
    case "admin_settings_documents":
      return MOCK_ADMIN_SETTINGS_DOCS;
    case "alert_notes":
      return MOCK_ALERT_NOTES;
    case "readings":
      return generateMockReadings();
    default:
      return [];
  }
};
        ...
        eq: (col: string, val: any) => {
          return {
            then: (onfulfilled?: any) =>
              Promise.resolve({ data: [patch], error: null }).then(onfulfilled),
          };
        },
        then: (onfulfilled?: any) =>
          Promise.resolve({ data: [patch], error: null }).then(onfulfilled),
    ...
    case "get_recent_visible_readings": {
      ...
      if (farmId && farmId !== "all") {
        const pondIds = new Set(
          MOCK_PONDS_FULL.filter((p) => p.farm_id === farmId).map((p) => p.id),
        );
